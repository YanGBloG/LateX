...
Về cơ bản, một mạng truyền thẳng có thể được thể hiện như sau:
	...
	\begin{figure}[h]
		...
		\begin{tikzpicture}[shorten >=1pt,->,draw=black!50, node distance=\layersep]
		    ...
		    % Annotate the layers
		    \node[annot,above of=H1-1, node distance=1cm] (hl1) {H1};
		    \node[annot,above of=H2-1, node distance=1cm] (hl2) {H2};
		    \node[annot,left of=hl2] {I-L};
		    \node[annot,right of=hl1] {O-L};
		\end{tikzpicture}
		\caption{Cấu trúc của một mạng truyền thẳng}
		\label{fig:feedforward-network}
	\end{figure}

Với Input, Output, I-L, O-L lần lượt là các dữ liệu đưa vào mạng, kết quả đạt được, phân lớp đầu vào, phân lớp đầu ra và các phân lớp ẩn H1, H2. Có thể thấy được mạng này có độ sâu là 3 (phân lớp đầu vào không được tính vào độ sâu của mạng), độ rộng là 5 với 2 phân lớp ẩn. Một mạng như vậy cũng được gọi là mạng liên kết toàn phần.\\
...
Có thể thấy được đầu ra của hàm $Sigmoid$ nằm trong khoảng $[0, 1]$ (Hình \ref{fig:tanh-sigmoid-function}), do đó hàm $Sigmoid$ thường được kết hợp với một số kĩ thuật khác để áp dụng cho bài toán phân loại nhị phân. Hiện tại, hàm $Sigmoid$ không còn được sử dụng nhiều cho các bài toán phức tạp do hiện tượng tiêu biến giá trị tối ưu trong quá trình huấn luyện của mạng.
	...
	\begin{figure}[h]
	    ...
	    \begin{tikzpicture}
		    ...
		    \addplot[color=blue]{(exp(x) - exp(-x)) / (exp(x) + exp(-x))};
	    	\addlegendentry{Tanh}
		    \end{axis}
	    \end{tikzpicture}
